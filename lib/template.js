...
Template.prototype._render = function(name, data, options) {
    ...
    let context = _.reduce(data, function(out, value, key) {
        ...
        if (value && value._transform instanceof Function) {
            //TODO _fn leaks as its used inside template
            _fn = out[key];
            out[key] = function() {
                return value._transform.call(this, _fn.call(this));
            };
        }
        return out;
    }, {});
...
Template._normalizeConfig = function normalizeProjectConfig(config) {
    ...
    /*
     * JSON.stringify always wraps string values in quotes, we need to get
     * rid of them in specific cases as we want to reference variables defined
     * in the template
     */
    Object.defineProperty(out.apps, '_transform', {
        value: function(str) {
            return str.replace(/bodyParser: "bodyParser"/g, 'bodyParser: bodyParser')
                .replace(/response: "response"/g, 'response: response');
        }
    })
    ...
    (config._apps || []).filter(function(appName) {
        ...
        out.apps[appName] = {
            baseUrl: config._host + ':' + config.listen[appName].port,
            listen: config.listen[appName].port,
            stopOnError: true,
            bodyParser: 'bodyParser',//variable reference (defined in template)
            response: 'response', //variable reference (defined in template)
        };
        ...
        if (   config._apps instanceof Array
            ...
            out.apps[appName].doc = {
                baseUrl: config._host + ':' + config.listen[appName+'-doc'].port,
                listen: config.listen[appName+'-doc'].port,
                title: 'Docs',
                stopOnError: true
            };
        }
