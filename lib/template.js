...
Template.prototype.initCmd = Promise.method(function(argv) {
    ...
    return this._getAnswers().bind(this).then(function(answers) {
        dependencies = normalizeProjectDependencies(answers._dependencies, answers);
        config = normalizeProjectConfig(answers._config);
        pkg = extractNpmPackage(answers);
        ...
        return npmInstall(dependencies.dependencies, ['--save'], argv.verbose);
    }).then(function() {
        return npmInstall(dependencies.devDependencies, ['--save-dev'], argv.verbose);
    });
});
...
Template.prototype.create = function(package, config, dependencies, license, verbosity) {
    ...
    return Promise.map(templates, function(template) {
        if (verbosity) {
            console.info(
                chalk.cyan('[INFO]') +
                ` Creating ${template.path.slice(process.cwd().length + 1)}`
            );
        }
 ...
 * @return {Object}
 */
function normalizeProjectDependencies(dependencies, answers) {
 ...
 * @return {Promise}
 */
function npmInstall(dependencies, args, verbosity) {
    args = _.clone(args) || [];
